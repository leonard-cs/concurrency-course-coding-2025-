...
class ScopedLock {
 public:
  ScopedLock(std::mutex& mutex) : mutex_(mutex) {
    mutex_.lock();  // This constructor will only return once the mutex
    // has been acquired.
  }

  // This is a *destructor*
  ~ScopedLock() {
    mutex_.unlock();  // No matter how the ScopedLock goes out of scope, this
    // destructor will get called.
  }
...
class Logger {
 public:
  ...
  void LogMessage(const std::string& message) {
    ScopedLock doesnotmatterwhatwecallit(mutex_);
